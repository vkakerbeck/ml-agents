...
`mlagentsdev-learn` using the `–curriculum` flag to point to the metacurriculum
...
```sh
mlagentsdev-learn config/trainer_config.yaml --curriculum=config/curricula/wall-jump/ --run-id=wall-jump-curriculum --train
```
