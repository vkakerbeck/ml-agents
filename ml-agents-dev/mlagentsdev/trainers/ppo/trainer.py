...
from mlagentsdev.envs import AllBrainInfo, BrainInfo
...
class PPOTrainer(Trainer):
    ...
    def __init__(self, brain, reward_buff_cap, trainer_parameters, training,
        ...
        super(PPOTrainer, self).__init__(brain, trainer_parameters,
                                         training, run_id)
        self.param_keys = ['batch_size', 'beta', 'buffer_size', 'epsilon', 'gamma', 'hidden_units', 'hidden_units_vec', 'lambd',
                           ...
                           'curiosity_enc_size', 'model_path', 'augment_r', 'forward_model_weight']
            ...
            if stored_info is not None:
                ...
                if not stored_info.local_done[idx]:
                    ...
                    if self.policy.use_vec_obs:
                        ...
                        self.vec_obs_collection[int(str(agent_id).split("-")[0])].append(stored_info.vector_observations[idx])
        ...
        for l in range(len(info.agents)):
            ...
            if ((info.local_done[l] or len(agent_actions) > self.trainer_parameters['time_horizon'])
                ...
                if info.local_done[l]:
                    ...
                    try:
                        if np.max(np.array(self.vec_obs_collection[int(str(agent_id).split("-")[0])])[:,1:-2])>0:
                            print("key collected by agent "+str(agent_id).split("-")[0]+" He reached floor "+str(np.max(np.array(self.vec_obs_collection[int(str(agent_id).split("-")[0])])[:,-1])))
