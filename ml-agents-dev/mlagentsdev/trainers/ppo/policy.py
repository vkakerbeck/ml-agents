...
from mlagentsdev.trainers.ppo.models import PPOModel
...
class PPOPolicy(Policy):
    ...
    def get_forward_predictions(self, curr_info, next_info):
        """
        Returns forward prediction of next state used for Curiosity-based training.
        :BrainInfo curr_info: Current BrainInfo.
        :BrainInfo next_info: Next BrainInfo.
        :return: Intrinsic rewards for all agents.
        """
        if self.use_curiosity:
            ...
            if self.use_recurrent:
                if curr_info.memories.shape[1] == 0:
                    curr_info.memories = self.make_empty_memory(len(curr_info.agents))
                feed_dict[self.model.memory_in] = curr_info.memories
            pred_next_state,encoded_next_state = self.sess.run([self.model.pred_next_state,self.model.encoded_next_state],
                                              feed_dict=feed_dict)
            return pred_next_state,encoded_next_state
        else:
            return None
